...
def upgrade() -> None:
    # Создаём enum-тип идемпотентно (checkfirst), а в колонке используем
    # create_type=False, чтобы create_table не пытался выпустить CREATE TYPE
    # повторно — иначе при остатке типа от прошлого запуска падает DuplicateObject.
